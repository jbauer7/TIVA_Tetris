...
{
	eeprom_byte_write(EEPROM_I2C_BASE, 0, (high_score & 0xFF));
	eeprom_byte_write(EEPROM_I2C_BASE, 1, ((high_score >> 8) & 0xFF));
	eeprom_byte_write(EEPROM_I2C_BASE, 2, ((high_score >> 16) & 0xFF));
	eeprom_byte_write(EEPROM_I2C_BASE, 3, ((high_score >> 24) & 0xFF));
}
...
void get_high_score(uint32_t *high_score)
{
	...
	eeprom_byte_read(EEPROM_I2C_BASE, 0, &read_byte);
	*high_score = read_byte;
	eeprom_byte_read(EEPROM_I2C_BASE, 1, &read_byte);
	*high_score |= (read_byte << 8);
	eeprom_byte_read(EEPROM_I2C_BASE, 2, &read_byte);
	*high_score |= (read_byte << 16);
	eeprom_byte_read(EEPROM_I2C_BASE, 3, &read_byte);
	*high_score |= (read_byte << 24);
}
